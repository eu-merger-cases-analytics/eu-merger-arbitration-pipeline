"""
Fetches one example row from raw.decision_hits and writes output to
...
def fetch_sample_row(conn) -> dict | None:
    """Returns the first row inserted into raw.decision_hits (lowest hit_id)."""
    with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
        cur.execute(
            f'SELECT * FROM {SCHEMA}.{TABLE} '
            f'ORDER BY "hit_id" LIMIT 1'
        )
        row = cur.fetchone()
...
def main() -> None:
    ...
    if row is None:
        ...
        out(
            ...
            f"(hit_id={row.get('hit_id')}, case={row.get('case_caseNumber')}):"
        )
        out()
        out(format_row(row))
